...
This project aims to support data analysis and machine learning tasks related to wind turbines. The following instructions will guide you on how to set up the project, run it in VS Code, and contribute by updating the Conda environment.
...
2. **Activate the Environment**:
   - After creating the environment, activate it with:
     ```bash
     conda activate wind_turbine_env
     ```

3. **Add the Environment to Jupyter**:
   - To ensure that the environment is available as a kernel in Jupyter notebooks, run:
     ```bash
     python -m ipykernel install --user --name=wind_turbine_env --display-name "Python (wind_turbine_env)"
     ```
...
3. **Choose the Correct Environment**:
   - In the list, look for `wind_turbine_env` with the specified Python version (e.g., `Python 3.8.x`). It should appear under **Conda Env**.
   - Click on `wind_turbine_env` to select it as the active environment for the notebook.

4. **Verify the Selection**:
   - Once selected, you should see "Python (wind_turbine_env)" displayed at the top of the notebook. This confirms that the notebook is using the correct Conda environment.

### Running the Project

1. **Run Cells**:
   - Use `Shift + Enter` to run individual cells in the notebook. Verify that each cell executes successfully to confirm that your environment is set up correctly.
...
2. **Update Environment for Others**:
   - After updating `environment.yml`, other contributors can sync their environment by running:
     ```bash
...
3. **Commit Your Changes**:
   - When you’ve made changes to the environment or code, commit and push them:
     ```bash
     git add .
     git commit -m "Add new dependencies to environment.yml"
